...
type
  ...
  TControlFiltroGride = class(TInterfacedObject, IControlFiltroGride)
  strict private
    FFrame: TFrame;
  private
    FDbGrid: TDbGrid;
  ...
  public
    class function New(AFrame: TFrame): IControlFiltroGride;
    procedure CriarPanelFiltro(Filtro: string);
    procedure InformarDbGrid(ADbGrid: TDbGrid);
    procedure Filtrar(Filtro: string);
    procedure LimparPesquisa;
...
procedure TControlFiltroGride.CriarPanelFiltro(Filtro: string);
begin
...
class function TControlFiltroGride.New(AFrame: TFrame): IControlFiltroGride;
begin
  Result := Self.Create(AFrame);
end;

function TControlFiltroGride.ObterPanel: TPanelCard;
begin
...
procedure TControlFiltroGride.CriarLabel(Panel: TPanelCard; Filtro: string);
begin
...
constructor TControlFiltroGride.Create(AFrame: TFrame);
begin
  FFrame := AFrame;
  FTextoFiltro := string.Empty;
end;

procedure TControlFiltroGride.CriarImagemFechar(Panel: TPanelCard);
begin
...
procedure TControlFiltroGride.ImageClick(Sender: TObject);
var
  ATask: iTask;
begin
...
procedure TControlFiltroGride.InformarDbGrid(ADbGrid: TDbGrid);
begin
  FDbGrid := ADbGrid;
  FDbGrid.OnDrawColumnCell := DBGrid1DrawColumnCell;
end;

procedure TControlFiltroGride.DestruirPanelsCard;
begin
  var Indice := 0;
...
procedure TControlFiltroGride.LimparPesquisa;
begin
  DestruirPanelsCard;
  Filtrar(string.Empty);
end;

procedure TControlFiltroGride.Filtrar(Filtro: string);
begin
...
procedure TControlFiltroGride.ArredondarCantos(componente: TWinControl);
begin
...
procedure TControlFiltroGride.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TControlFiltroGride.DrawTextDestacado(Canvas : TCanvas; Rect: TRect;
...
procedure TControlFiltroGride.OrdernarLista(var Lista: TList<string>; Texto: string);
var
  ValorComeco: Integer;
  ValorFinal: Integer;
begin
...
function TControlFiltroGride.GetCamposComFiltro(TextoFiltro: string): string;
...
function TControlFiltroGride.GetCaptionFiltro: TList<string>;
begin
...
function TControlFiltroGride.GetFiltroAplicado: string;
begin
  var Retorno := string.Empty;
  var Lista := GetCaptionFiltro;
  for var Indice := 0 to Pred(Lista.Count) do
